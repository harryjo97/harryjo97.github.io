...
주어진 weighted adjacency matrix $W$ 에 대해 degree matrix  $D$ 는 $D_{ii} = \sum^{N}_{j=1} W_{ij}$ 를 만족하는 diagonal matrix 로 정의합니다. 쉽게 말해, $D_{ii}$ 는 vertex $i$ 를 끝점으로 가지는 edge 들의 weight 를 모두 더한 값과 같습니다. 위의 예시처럼 edge 마다 weight 를 1로 설정한다면, degree matrix 의 diagonal element 는 각 vertex 의 degree 를 의미하기 때문에 degree matrix 라는 명칭이 붙었습니다.
...
$$
off-diagonal 의 원소와 diagonal 의 원소를 나눠서 계산해보겠습니다.
